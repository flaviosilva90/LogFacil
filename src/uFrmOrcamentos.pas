...

{
  uFrmOrcamentos.pas
  Orçamentos de Venda.
  Fluxo: ABERTO → CONVERTIDO (ao gerar Venda) | EXPIRADO | CANCELADO
  A tela de Vendas pode importar um orçamento aberto para gerar a venda.
}
...
type
  TFrmOrcamentos = class(TForm)
    ...
    lblValidade: TLabel;    edtValidade: TDateTimePicker;
...
begin
  ...
  sgdLista.Cells[4,0]:='Validade'; sgdLista.Cells[5,0]:='Total';
  sgdLista.Cells[6,0]:='Status'; sgdLista.Cells[7,0]:='Nº Venda';
  { Grid itens }
  ...
  cboStatus.Items.Add(STO_ABERTO);
  cboStatus.Items.Add(STO_CONVERTIDO);
  cboStatus.Items.Add(STO_EXPIRADO);
  cboStatus.Items.Add(STO_CANCELADO);
  cboStatus.ItemIndex := 0;
  ModoEdicao(False);
  CarregarLista;
end;

procedure TFrmOrcamentos.CarregarLista;
var Qry: TFDQuery; W: string;
begin
  Qry := DMPrincipal.NovaQuery;
  try
    Qry.SQL.Text :=
      'SELECT O.ID,O.NUMERO,C.NOME,O.DT_EMISSAO,O.DT_VALIDADE,' +
      'O.VL_TOTAL,O.STATUS,O.ID_VENDA ' +
      'FROM ORCAMENTOS O JOIN CLIENTES C ON C.ID=O.ID_CLIENTE ';
    W := '';
    if cboStatus.ItemIndex > 0 then
      W := 'O.STATUS=''' + cboStatus.Items[cboStatus.ItemIndex] + '''';
    if Trim(edtFiltro.Text) <> '' then
    begin
      if W <> '' then W := W + ' AND ';
      W := W + '(UPPER(C.NOME) CONTAINING UPPER(:F) OR CAST(O.NUMERO AS VARCHAR(10)) CONTAINING :F)';
    end;
    if W <> '' then Qry.SQL.Add('WHERE '+W);
    Qry.SQL.Add('ORDER BY O.NUMERO DESC');
...
procedure TFrmOrcamentos.PreencherGridLista(Qry: TFDQuery);
var R: Integer;
begin
...
procedure TFrmOrcamentos.CarregarItens(AID: Integer);
...
begin
  ...
  try
    Qry.SQL.Text:=
      'SELECT OI.ITEM,OI.ID_PRODUTO,P.DESCRICAO,U.SIGLA,' +
      'OI.QUANTIDADE,OI.VL_UNITARIO,OI.VL_TOTAL ' +
      'FROM ORCAMENTOS_ITENS OI ' +
      'JOIN PRODUTOS P ON P.ID=OI.ID_PRODUTO ' +
      'JOIN UNIDADES U ON U.ID=P.ID_UNIDADE ' +
      'WHERE OI.ID_ORCAMENTO=:ID ORDER BY OI.ITEM';
    Qry.ParamByName('ID').AsInteger:=AID; Qry.Open;
...
procedure TFrmOrcamentos.RecalcularTotais;
...
procedure TFrmOrcamentos.ModoEdicao(AEditar: Boolean);
begin
...
procedure TFrmOrcamentos.LimparEdicao;
var R: Integer;
begin
  FIDSelecionado:=0; FStatusAtual:='';
  edtNumero.Text:='(automático)';
  edtClienteID.Text:=''; edtClienteNome.Text:='';
  edtEmissao.Date:=Date; edtValidade.Date:=Date+30;
  edtStatusLabel.Text:=STO_ABERTO; edtCondPagto.Text:=''; mmoObs.Lines.Clear;
  sgdItens.RowCount:=2;
...
procedure TFrmOrcamentos.AtualizarBotoes;
var EhNovo: Boolean;
begin
  EhNovo := FIDSelecionado = 0;
  btnSalvar.Enabled     := pnlEdicao.Visible and ((FStatusAtual='') or (FStatusAtual=STO_ABERTO));
  btnConverter.Enabled  := pnlEdicao.Visible and (FStatusAtual=STO_ABERTO) and not EhNovo;
  btnCancelarDoc.Enabled:= pnlEdicao.Visible and (FStatusAtual=STO_ABERTO) and not EhNovo;
  btnAddItem.Enabled    := EhNovo or (FStatusAtual=STO_ABERTO);
  btnRemItem.Enabled    := EhNovo or (FStatusAtual=STO_ABERTO);
  btnBuscarProd.Enabled := EhNovo or (FStatusAtual=STO_ABERTO);
end;

procedure TFrmOrcamentos.btnBuscarClick(Sender: TObject);
...
procedure TFrmOrcamentos.sgdListaClick(Sender: TObject);
...
begin
  ...
  try
    Qry.SQL.Text:=
      'SELECT O.*,C.NOME AS CNOME FROM ORCAMENTOS O ' +
      'JOIN CLIENTES C ON C.ID=O.ID_CLIENTE WHERE O.ID=:ID';
    ...
    if not Qry.FieldByName('DT_VALIDADE').IsNull then
      edtValidade.Date:=Qry.FieldByName('DT_VALIDADE').AsDateTime;
...
procedure TFrmOrcamentos.btnBuscarCliClick(Sender: TObject);
...
procedure TFrmOrcamentos.btnBuscarProdClick(Sender: TObject);
var SID: string; ID: Integer; Qry: TFDQuery;
begin
  ...
  try
    Qry.SQL.Text:=
      'SELECT P.ID,P.DESCRICAO,U.SIGLA,P.PRECO_VENDA ' +
      'FROM PRODUTOS P JOIN UNIDADES U ON U.ID=P.ID_UNIDADE WHERE P.ID=:ID AND P.ATIVO=''S''';
    Qry.ParamByName('ID').AsInteger:=ID; Qry.Open;
...
procedure TFrmOrcamentos.btnAddItemClick(Sender: TObject);
...
procedure TFrmOrcamentos.btnRemItemClick(Sender: TObject);
var R,I: Integer;
begin
  R:=sgdItens.Row;
...
procedure TFrmOrcamentos.edtFreteChange(Sender: TObject);
begin RecalcularTotais; end;

procedure TFrmOrcamentos.edtDescGlobalChange(Sender: TObject);
begin RecalcularTotais; end;

procedure TFrmOrcamentos.btnSalvarClick(Sender: TObject);
var Qry: TFDQuery; IDCli,IDOrc,NItem,R: Integer;
  ...
  try
    ...
    try
      if FIDSelecionado=0 then
        Qry.SQL.Text:=
          'INSERT INTO ORCAMENTOS (ID_CLIENTE,ID_USUARIO,DT_EMISSAO,DT_VALIDADE,STATUS,' +
          'VL_PRODUTOS,VL_FRETE,VL_DESCONTO,VL_TOTAL,COND_PAGTO,OBS) ' +
          'VALUES (:IC,:IU,:EM,:VL,:ST,:VP,:VF,:VD,:VT,:CP,:OB) RETURNING ID'
      else
        Qry.SQL.Text:=
          'UPDATE ORCAMENTOS SET ID_CLIENTE=:IC,DT_EMISSAO=:EM,DT_VALIDADE=:VL,' +
          'VL_PRODUTOS=:VP,VL_FRETE=:VF,VL_DESCONTO=:VD,VL_TOTAL=:VT,' +
          'COND_PAGTO=:CP,OBS=:OB WHERE ID=:ID';
      Qry.ParamByName('IC').AsInteger  :=IDCli;
      Qry.ParamByName('IU').AsInteger  :=SessaoID;
      Qry.ParamByName('EM').AsDateTime :=edtEmissao.Date;
      Qry.ParamByName('VL').AsDateTime :=edtValidade.Date;
      if FIDSelecionado=0 then Qry.ParamByName('ST').AsString:=STO_ABERTO;
      ...
      if FIDSelecionado=0 then
      begin Qry.Open; IDOrc:=Qry.FieldByName('ID').AsInteger; end
      else
      begin
        Qry.ParamByName('ID').AsInteger:=FIDSelecionado;
        Qry.ExecSQL; IDOrc:=FIDSelecionado;
        DMPrincipal.ExecSQL('DELETE FROM ORCAMENTOS_ITENS WHERE ID_ORCAMENTO='+IntToStr(IDOrc));
      end;
      for R:=1 to sgdItens.RowCount-1 do
      begin
        ...
        Qry.Close;
        Qry.SQL.Text:=
          'INSERT INTO ORCAMENTOS_ITENS (ID_ORCAMENTO,ITEM,ID_PRODUTO,QUANTIDADE,VL_UNITARIO,VL_DESCONTO,VL_TOTAL) ' +
          'VALUES (:IO,:IT,:IP,:QT,:VU,:VD,:VT)';
        Qry.ParamByName('IO').AsInteger:=IDOrc;
      ...
      if FIDSelecionado=0 then
      begin
        FIDSelecionado:=IDOrc; FStatusAtual:=STO_ABERTO;
        edtNumero.Text:=IntToStr(IDOrc); edtStatusLabel.Text:=STO_ABERTO;
      end;
...
procedure TFrmOrcamentos.btnConverterClick(Sender: TObject);
{ Converte orçamento em venda e abre a tela de Vendas já preenchida }
var Qry: TFDQuery; IDVenda,R: Integer;
    VlProd,VlFrete,VlDesc,VlTotal,Qtd,VlUnit,VlTotal2: Double;
begin
  if FIDSelecionado=0 then Exit;
  if MessageDlg('Converter orçamento em Venda?',mtConfirmation,[mbYes,mbNo],0)<>mrYes then Exit;
  Qry:=DMPrincipal.NovaQuery;
  try
    DMPrincipal.IniciarTran;
    try
      { Busca dados do orçamento }
      Qry.SQL.Text:='SELECT * FROM ORCAMENTOS WHERE ID=:ID';
      Qry.ParamByName('ID').AsInteger:=FIDSelecionado; Qry.Open;
      if Qry.IsEmpty then Exit;
      VlProd  :=Qry.FieldByName('VL_PRODUTOS').AsFloat;
      VlFrete :=Qry.FieldByName('VL_FRETE').AsFloat;
      VlDesc  :=Qry.FieldByName('VL_DESCONTO').AsFloat;
      VlTotal :=Qry.FieldByName('VL_TOTAL').AsFloat;
      { Cria venda como ORCAMENTO }
      Qry.Close;
      Qry.SQL.Text:=
        'INSERT INTO VENDAS (ID_CLIENTE,ID_USUARIO,DT_EMISSAO,STATUS,' +
        'VL_PRODUTOS,VL_FRETE,VL_DESCONTO,VL_TOTAL,COND_PAGTO,OBS) ' +
        'SELECT ID_CLIENTE,:IU,CURRENT_DATE,''ORCAMENTO'',' +
        'VL_PRODUTOS,VL_FRETE,VL_DESCONTO,VL_TOTAL,COND_PAGTO,' +
        '''Convertido do Orçamento Nº ''||NUMERO ' +
        'FROM ORCAMENTOS WHERE ID=:ID RETURNING ID';
      Qry.ParamByName('IU').AsInteger:=SessaoID;
      Qry.ParamByName('ID').AsInteger:=FIDSelecionado;
      Qry.Open;
      IDVenda:=Qry.FieldByName('ID').AsInteger;
      { Copia itens }
      Qry.Close;
      Qry.SQL.Text:=
        'INSERT INTO VENDAS_ITENS (ID_VENDA,ITEM,ID_PRODUTO,QUANTIDADE,VL_UNITARIO,VL_DESCONTO,VL_TOTAL) ' +
        'SELECT :IV,ITEM,ID_PRODUTO,QUANTIDADE,VL_UNITARIO,VL_DESCONTO,VL_TOTAL ' +
        'FROM ORCAMENTOS_ITENS WHERE ID_ORCAMENTO=:IO';
      Qry.ParamByName('IV').AsInteger:=IDVenda;
      Qry.ParamByName('IO').AsInteger:=FIDSelecionado;
      Qry.ExecSQL;
      { Atualiza orçamento como CONVERTIDO }
      DMPrincipal.ExecSQL(
        'UPDATE ORCAMENTOS SET STATUS=''CONVERTIDO'', ID_VENDA='+IntToStr(IDVenda)+
        ' WHERE ID='+IntToStr(FIDSelecionado));
      DMPrincipal.CommitTran;
      FStatusAtual:=STO_CONVERTIDO;
      edtStatusLabel.Text:=STO_CONVERTIDO;
      IDOrcamentoConvertido:=FIDSelecionado;
      AtualizarBotoes;
      CarregarLista;
      ShowMessage(Format('Orçamento convertido em Venda Nº %d com sucesso!'#13#10+
                         'Acesse Movimentos → Vendas para finalizar a venda.', [IDVenda]));
    except on E: Exception do begin DMPrincipal.RollbackTran; ShowMessage('Erro: '+E.Message); end; end;
  finally Qry.Free; end;
end;

procedure TFrmOrcamentos.btnCancelarDocClick(Sender: TObject);
begin
  if FIDSelecionado=0 then Exit;
  if MessageDlg('Cancelar este orçamento?',mtConfirmation,[mbYes,mbNo],0)<>mrYes then Exit;
  try
    DMPrincipal.IniciarTran;
    DMPrincipal.ExecSQL('UPDATE ORCAMENTOS SET STATUS=''CANCELADO'' WHERE ID='+IntToStr(FIDSelecionado));
    DMPrincipal.CommitTran;
    FStatusAtual:=STO_CANCELADO; edtStatusLabel.Text:=STO_CANCELADO;
...
begin
  if pnlEdicao.Visible then begin ModoEdicao(False); LimparEdicao; end
  else Close;
end;
